...
def save_series_to_csv(series, fileName, seriesName):
	path = "results/SARIMAX/" + originFileName[:-4]
	...
	path = "results/SARIMAX/" + originFileName[:-4] + "/" + seriesName
...
def main(seriesName):
	...
	# exo
	exo = series.drop(labels=seriesName, axis=1)
...
if __name__ == '__main__':
	...
	for house in houses:
		...
		for procces in proc:
			process.join()
			#seriesNames = appliance
			#main(seriesNames)
		#with concurrent.futures.ProcessPoolExecutor() as executor:
		#    executor.map(main, list(appliances.get(house)))
